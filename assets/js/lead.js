...
document.getElementById('leadForm').addEventListener('submit', async e=>{
  ...
  if(!name || !/^\d{6,15}$/.test(mobile)){ statusEl.textContent='Please provide valid Name and Mobile'; return; }

  statusEl.textContent='Submitting…';
  const timezone   = Intl.DateTimeFormat().resolvedOptions().timeZone || '';
  const deviceType = detectDeviceType();              // <-- device type (Mobile/Tablet/Desktop)
  const ip         = await getIP();

  const payload={ isAgent, name, mobile, city, company, timezone, ip, deviceType, project, unit };

  try{
    const res = await apiPost('addlead', payload); // uses text/plain in app.js to avoid preflight
